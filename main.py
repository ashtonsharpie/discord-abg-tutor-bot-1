...
def get_conversation_response(user_input: str, user_id: int) -> str:
    ...
    elif 'ap' in lowered or any(word in lowered for word in ['biology', 'chemistry', 'physics', 'calculus', 'history']):
...
async def send_message(message: Message, user_message: str) -> None:
    ...
    try:
        ...
        # Always send in the channel (removed DM functionality)
...
async def on_message(message: Message) -> None:
    ...
    print(f"[DEBUG] Starts with !: {message.content.startswith('!')}")
    print(f"[DEBUG] User in conversation: {user_id in conversation_active}")

    # Handle ! commands FIRST (ALWAYS responds, STARTS conversation)
    if message.content.startswith('!'):
        print(f"[DEBUG] ! command detected")
        ...
        print(f"[DEBUG] Response from get_response: {response[:50] if response else 'None'}...")
        if response:
            ...
            if user_id not in conversation_active or not conversation_active[user_id]:
                ...
                print("[DEBUG] Starting conversation and replying")
                await message.reply(response + CONVERSATION_START_MSG, mention_author=False)
            else:
                print("[DEBUG] Continuing conversation and replying")
    ...
    if client.user.mentioned_in(message) or isinstance(message.channel, DMChannel):
        ...
        if response:
            # Use channel.send for DMs (reply doesn't work in DMs)
    ...
    if user_id in conversation_active and conversation_active[user_id]:
        print(f"[DEBUG] Continuing conversation for user {user_id}")
        response = get_conversation_response(message.content, user_id)

        # Check if goodbye was said (user removed from conversation_active)
        if user_id not in conversation_active or not conversation_active[user_id]:
            # They said goodbye, just send the response and end
            print(f"[DEBUG] Goodbye detected, ending conversation")
            if response:
                await message.reply(response, mention_author=False)
        else:
    ...
    print(f"[DEBUG] Message ignored - user {user_id} not in active conversation")
